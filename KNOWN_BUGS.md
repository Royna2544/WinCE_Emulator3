...
- Direct synthetic-disk CE `fmd.h` metadata IOCTLs are covered for empty reserved-table, raw-block-size, and deterministic `FMDInfo` responses; real flash hardware interface, block lock/unlock, raw write, reserved-region read/write, and region-table behavior remain open.
