...
- Current live-launch diagnostics show a stronger remote-server failure:
  `RemoteServer::start` prints the bound URL, but no TCP listener is visible on
  port `8765` while the emulator process is alive. Until the accept thread or
  server handle lifetime is fixed, REST screenshots/input/sensor injection can
  fail before the guest UI is reachable.
...
- Remote-driven iNavi startup now advances past the real `happyway_win` modal
  ...
  button. The app still remains on the animated splash while loading
  resources/map data. Current evidence points at repeated file-position/resource
  loading around `iNavi.exe+0x642f8` (`SetFilePointer`, `COREDLL` ordinal 173)
  after map/resource files are opened. The guest now reaches `COM7:` plus
  `MFS1:`/`SMB1:` device initialization, but normal map UI transition and
  posted-touch dispatch are still not observed.
