...
- Direct synthetic-disk CE `fmd.h` IOCTLs are covered for XIP mode state, block lock/unlock state, sector-size state, raw block writes, empty reserved-table, raw-block-size, and deterministic `FMDInfo` responses; real flash hardware interface, reserved-region read/write, and region-table behavior remain open.
