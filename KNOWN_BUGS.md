...
- Direct synthetic-disk `FSDMGR_ReadDisk @25`/`FSDMGR_WriteDisk @35` now cover the CE zero-sector no-buffer edge; remaining direct disk read/write risk is limited to real physical block-driver forwarding behavior.
...
- Direct synthetic-disk CE `fmd.h` IOCTLs are covered for XIP mode state, block lock/unlock state, a 56-byte `FMDInterface` ABI skeleton with null callback slots, sector-size state, total-block-bounded raw block writes, CE `ReservedReq` validation plus named synthetic reserved-region read/write persistence and `ReservedEntry` table output, `FlashRegion` table validation/storage, raw-block-size, and deterministic `FMDInfo` responses with stored region and reserved-entry counts; real flash hardware interface, hardware-backed FLS discovery/forwarding, and callable FMD callback behavior remain open.
...
- iNavi fast live startup now clears the heavy map/search DB I/O phase quickly
  with the generic large read-only memory backing and the host idle-slice guard
  now prevents stale parked `GetMessage` waiters from forcing 100 ms active CPU
  slices, but the app-owned splash window still remains visible after loader
  waiters drain. Remote touch is delivered to the real popup window, so the
...
- Remote-driven iNavi startup now advances past the real `happyway_win` modal
  ...
  z-order demotion for `0x00020008` has been observed. The later live
  active-thread wedge at `THREAD_EXIT_STUB_ADDR` (`0x7fffffe0`) is fixed by
...
- The earlier startup-slow path caused by existing read/write map/search files
  ...
  opens rose during resource/map loading. The later false 100 ms live-pump
  slicing from stale parked message waiters is also closed. The remaining
  visible symptom is not a black splash or missing touch delivery. The real
  splash popup `0x00020008` remains topmost and receives the remote tap
  messages, so the underlying map child windows cannot react until the app
  hides, destroys, or demotes that owned popup.
