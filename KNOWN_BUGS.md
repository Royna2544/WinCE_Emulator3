...
- GDI object-selection parity still needs broader complex-region and cross-DC validation; the CE `SelectObject` simple-region `SIMPLEREGION` return/status clipping path and `SelectClipRgn` copy/lifetime behavior are now covered.
...
- Remote-driven iNavi startup now advances past the real `happyway_win` modal
  ...
  orphaned WNDPROC return stubs can recover from the last matching frame. The
