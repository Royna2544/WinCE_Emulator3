...
- Direct FSDMGR registry helper imports `@18` through `@20` are covered for the CE missing-value status/output-clearing surface; real logical-disk registry-root lookup and cache DLL/filter behavior remain open.
...
- Direct CE `FSDMGR_GetRegistryFlag @18`, `FSDMGR_GetRegistryString @19`, and
  ...
  fail-closed path and output clearing; real disk-profile registry lookup and
  utility/cache/filter DLL metadata forwarding remain open.
