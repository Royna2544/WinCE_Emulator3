...
- GDI object-selection parity still needs broader complex-region and lifetime validation; the CE `SelectObject` simple-region `SIMPLEREGION` return/status clipping path is now covered.
...
- Polygon/polyline parity now covers CE invalid-HDC, null-point, and small-count validation, including `Polyline` count-zero no-op and `Polygon` count-failure last-error behavior; broader stress-shape clipping, fill-mode, and pathological point-list fidelity remain open.
- Rectangle/ellipse/round-rect parity now covers CE invalid-HDC handling, but deeper stress-shape clipping, viewport, and writable-bitmap fidelity remain open.
...
- Remote-driven iNavi startup now advances past the real `happyway_win` modal
  ...
  targets instead of consuming/reposting them, and direct-send WNDPROC cleanup
  now preserves live callouts until the saved PC reaches their return PC. The
