...
- Public `LockFileEx`/`UnlockFileEx` now resolve at the CE coredll ordinals and validate real file handles plus readable `OVERLAPPED*` wrappers, but byte-range lock ownership/conflict enforcement is still not implemented for host-backed FSD files.
