...
- Rectangle/ellipse/round-rect parity now covers CE invalid-HDC handling, and raw `LineTo`/`Polyline`/`Polygon` plus `Rectangle`/`Ellipse`/`RoundRect` now apply the selected-DIB viewport origin, but deeper stress-shape clipping, window-origin/viewport-ext APIs, and writable-bitmap fidelity remain open.
