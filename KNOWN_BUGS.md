...
- CE `TransparentBltTransparencyTest`-style selected memory DC drawing into `CreateBitmap(..., NULL)` is now covered for the raw 1/2/4/8/16/24/32 bpp sweep: `SelectObject` materializes owned backing before `FillRect`/`SetPixel`, and `TransparentImage` observes those pixels through the source HDC. Remaining transparent-blit clipping/error edge cases are still open.
