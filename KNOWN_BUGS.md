...
- Direct synthetic-disk CE `fmd.h` IOCTLs are covered for XIP mode state, block lock/unlock state, sector-size state, raw block writes, CE `ReservedReq` validation against the empty reserved table, raw-block-size, and deterministic `FMDInfo` responses; real flash hardware interface, actual reserved-region storage/read/write, and region-table behavior remain open.
