...
- Raw `GetIconInfo` now distinguishes stock cursor handles from stock icon handles for CE `ICONINFO.fIcon` output and writes tracked bitmap-backed icon hotspot/mask/color fields, but broader caller-owned bitmap lifetime and live guest consumer paths still need validation.
