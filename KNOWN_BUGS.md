...
- Direct synthetic-disk CE `fmd.h` IOCTLs are covered for XIP mode state, block lock/unlock state, empty reserved-table, raw-block-size, and deterministic `FMDInfo` responses; real flash hardware interface, raw write, reserved-region read/write, and region-table behavior remain open.
