...
- Direct `FSDMGR_AdvertiseInterface @2` is covered for the shared coredll wrapper path, including GUID/name validation, FSDMGR empty-name root mapping, advertisement add/remove storage, configured block-device `IClass` publication/removal for `\StoreMgr\<device>` names, coredll `EnumDeviceInterfaces @1874` enumeration of the tracked advertisement table with CE GUID/name/size/error behavior, CE message queues (`CreateMsgQueue`/`OpenMsgQueue`/`ReadMsgQueue`/`WriteMsgQueue`/`GetMsgQueueInfo`/`CloseMsgQueue`) with CE access-direction, full-queue timeout, oversized-write, truncated-read, `MSGQUEUE_ALLOW_BROKEN` pipe-disconnect behavior, and `MSGQUEUE_MSGALERT` priority/single-slot behavior, and coredll `RequestDeviceNotifications @1504`/`StopDeviceNotifications @1505` typed subscription handles with `DEVDETAIL` attach/detach delivery; true per-device `fsdev_t` scoping and message-queue blocking-wait edge cases remain open.
...
- Direct `FSDMGR_AsyncEnterVolume @80` and `FSDMGR_AsyncExitVolume @81` imports are covered for registered synthetic HVOL lock-token success/failure paths; real CE mounted-volume enter/exit lifetime accounting remains open.
- Direct `FSDMGR_ParseSecurityDescriptor @82` is covered for CE private descriptor-size extraction and validation; broader file ACL persistence/enforcement remains open.
...
- `ExtEscape` now covers the CE display-driver query/protected-error surface
  ...
  the uncached framebuffer base address, byte/pitch strides, and dimensions.
  ...
  video protection, guest-readable direct framebuffer memory behind the
  returned pointer, and broader DISPPERF coverage outside those raw draw paths
