...
fn bitmap_pixel_rgb_from_memory<M: CoredllGuestMemory>(
    memory: &M,
    bitmap: &crate::ce::resource::BitmapObject,
    x: i32,
    y: i32,
    ...
    let x = x as u32;
    let row_start = row.checked_mul(bitmap.width_bytes as u32)?;
    match bitmap.bits_pixel {
        1 => {
        ...
        8 => {
            let addr = bitmap.bits_ptr.wrapping_add(row_start).wrapping_add(x);
            memory
                .read_u8(addr)
                .ok()
                .map(|index| rgb_from_color_table(bitmap, index))
        ...
        16 => {
            ...
            let raw = u32::from(memory.read_u16(addr).ok()?);
            if let Some([red_mask, green_mask, blue_mask]) = bitmap.rgb_masks {
                Some([
                    component_from_mask(raw, red_mask)?,
                    component_from_mask(raw, green_mask)?,
        ...
        24 => {
            let addr = bitmap.bits_ptr.wrapping_add(row_start).wrapping_add(x * 3);
...
fn bitmap_pattern_bytes<M: CoredllGuestMemory>(
    ...
    for index in 0..len {
        bytes.push(
            memory
                .read_u8(bitmap.bits_ptr.wrapping_add(index as u32))
                .ok()?,
        );
    }
    Some(bytes)
...
fn pattern_pixel_rgb(
    ...
    let px = (x - origin.x).rem_euclid(bitmap.width);
    let py = (y - origin.y).rem_euclid(bitmap.height);
    bitmap_pixel_rgb(bitmap, bytes, px, py)
}
...
fn set_brush_org_ex_raw<M: CoredllGuestMemory>(
    ...
    let y = raw_i32_arg(args, 2);
    let out_ptr = raw_arg(args, 3);
    let Some(previous) = kernel.resources.set_brush_origin(hdc, Point { x, y }) else {
        kernel
            .threads
            .set_last_error(thread_id, ERROR_INVALID_HANDLE);
        return false;
...
fn draw_polyline_to_bitmap_for_hdc<M: CoredllGuestMemory>(
    kernel: &CeKernel,
    memory: &mut M,
    hdc: u32,
    points: &[Point],
...
fn polygon_raw<M: CoredllGuestMemory>(
    kernel: &mut CeKernel,
    memory: &mut M,
    framebuffer: Option<&mut dyn Framebuffer>,
    thread_id: u32,
...
fn rectangle_raw<M: CoredllGuestMemory>(
    kernel: &mut CeKernel,
    memory: &mut M,
...
const R2_NOTCOPYPEN: i32 = 4;
const R2_MASKPENNOT: i32 = 5;
const R2_NOT: i32 = 6;
const R2_XORPEN: i32 = 7;
const R2_NOTMASKPEN: i32 = 8;
const R2_MASKPEN: i32 = 9;
const R2_NOTXORPEN: i32 = 10;
const R2_NOP: i32 = 11;
const R2_MERGENOTPEN: i32 = 12;
const R2_COPYPEN: i32 = 13;
const R2_MERGEPENNOT: i32 = 14;
