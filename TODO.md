...
- Extend `ExtEscape` beyond the now-covered CE `QUERYESCSUPPORT` display-driver query surface, raw-framebuffer unsupported query, invalid query-buffer validation, invalid-HDC validation, privileged-display-escape `ERROR_INVALID_ACCESS` edges, and zeroed CE-sized `DISPPERF_EXTESC_*` payload contract by implementing real gamma, rotation, video-protection, framebuffer access, and nonzero display-performance instrumentation counters.
...
- Extend `StringCompress`/`StringDecompress` beyond the now-covered CE `compr2.c`
  all-zero and shorter raw even/odd stream packet paths by implementing the
  opaque `CECompress`/`CEDecompress` payload branch.
...
- Continue the remote-driven iNavi startup investigation from the current
  ...
  handles the modal button, direct-send WNDPROC cleanup now preserves live
  callouts until the saved PC reaches their return PC, post-dialog taps reach
  iNavi, and the app opens/reads mapdata, resource, font, and config files.
