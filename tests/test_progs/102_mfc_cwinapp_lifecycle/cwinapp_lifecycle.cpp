...
#define ID_FIXTURE_COMMAND 10201
...
BEGIN_MESSAGE_MAP(CFixtureWnd, CWnd)
    ...
    ON_MESSAGE(WM_USER + 102, OnUser)
END_MESSAGE_MAP()

int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrev, LPWSTR cmd, int show) {
    if (!AfxWinInit(hInstance, hPrev, cmd, show)) return FixtureFail(10201);

    CString cls = AfxRegisterWndClass(0);
    CFixtureWnd wnd;
    if (!wnd.CreateEx(0, cls, _T("mfc fixture 102"), WS_VISIBLE, CRect(0,0,120,80), NULL, 0))
        return FixtureFail(10202);

    if ((wnd.seen & 1) == 0) return FixtureFail(10203);
    if (wnd.SendMessage(WM_USER + 102, 7, 0) != 0x06607) return FixtureFail(10204);
    if ((wnd.seen & 2) == 0) return FixtureFail(10205);

    wnd.SendMessage(WM_COMMAND, ID_FIXTURE_COMMAND, 0);
    if ((wnd.seen & 8) == 0) return FixtureFail(10206);
