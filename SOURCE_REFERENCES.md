...
- `draw.cpp::ClipBitBlt(ETransparentImage)` uses the shared clipping matrix with
  ...
  same-framebuffer `TransparentImage` now cover the off-left and off-top cases
  where clipping the destination must also advance the sampled source coordinate
  or row while still skipping source pixels that match the transparent color.
