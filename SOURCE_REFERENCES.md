...
- Shell file-change notification APIs:
  ...
  - `winbase.h` declares `FindFirstChangeNotificationW`,
    ...
    `UNLOCK_BLOCKS`, `SET_SECTORSIZE`, `RAW_WRITE_BLOCKS`,
    `GET_RESERVED_TABLE`, `GET_RAW_BLOCK_SIZE`, and `GET_INFO`, storing a
    BOOLEAN XIP flag, consuming the 8-byte `BlockLockInfo` range payload,
    accepting a DWORD sector-size state, validating the 16-byte
    `RawWriteBlocksReq` buffer-size contract before block writes, returning an
    empty table size when no reserved table exists, a DWORD raw block size, and
    flash type/base/region/reserved-count metadata.
    ...
    disk backing, empty reserved-table, raw-block-size, and deterministic
    NOR-style `FMDInfo` metadata for the FMD controls.
    ...
    cache/filter DLL behavior, remaining hardware-flash/FMD interface/reserved-region/region-table and specialized disk IOCTLs, real FATFS
...
- Explorer/COREDLL startup ordinals:
  ...
  - `syncobj.c` anchors `OpenEventW` as an existing named event open with
    ...
    `CESHELL\API\api.cpp` writes `psfi->iIcon` whenever either `SHGFI_ICON` or
    `SHGFI_SYSICONINDEX` is requested, writes `psfi->hIcon` only for
    `SHGFI_ICON`, writes `psfi->dwAttributes` only for `SHGFI_ATTRIBUTES`, and
    returns a system image-list handle whenever either `SHGFI_ICON` or
    `SHGFI_SYSICONINDEX` is requested, choosing the small list only when
    `SHGFI_SMALLICON` is present. Explorer `taskbar.cpp` obtains and destroys
    both empty-path large and small system image lists during taskbar cleanup.
    Raw `SHGetFileInfoW` now returns distinct large/small system-list handles
    for those queries, populates `SHFILEINFO.iIcon` for icon-only calls, leaves
    unrequested `hIcon`/`iIcon`/`dwAttributes` fields untouched, and raw
    `ImageList_Destroy` accepts the synthetic system handles as a successful
    release while preserving the shared emulator fallback.
...
- GWE class/message surface:
  ...
  - GWE now keeps a receiver-side sent-message queue distinct from posted
    ...
    dispatch stale work after the sender has resumed. Focused Unicorn
    regressions now cover the direct import re-entry path as well, including
    ...
    plus the direct `smfResultReady` replay path that writes `lpdwResult`,
    consumes the completed send state, and resumes the original sender context
    with success.
...
- GDI DIB/color-table and DIB brush surface:
  ...
  - CE `draw.cpp::TransparentBltPatBltTest` seeds source and destination
    ...
    as the other blit paths, including isolated negative source-width and
    source-height regressions that prove horizontal and vertical source
    mirroring through the color-key path without a simultaneous destination
    flip.
  - CE `draw.cpp::passNull2Draw(EMaskBlt)` expects `MaskBlt` to fail null/bad
    ...
    destination-height and combined width/height regressions, all-edge
    selected-DIB/framebuffer clip regressions that preserve source/mask
    alignment when destination
...
- CE file namespace / SDMMC mount precedent:
  ...
  - `winbase.h` declares `GetDiskFreeSpaceExW` with three `ULARGE_INTEGER`
    ...
    wait-event readiness and wake signaling, `MSGQUEUE_MSGALERT`
    priority/single-slot behavior, and Unicorn blocking `ReadMsgQueue` /
    `WriteMsgQueue` timeout parking with CE-style replay on readiness or
    timeout; full CE `fsdev_t` per-device scoping remains queued.
    ...
    for FSDMGR cache imports. Physical block-driver backing, remaining hardware-flash/FMD interface/reserved-region/region-table and disk IOCTL
...
- CE GDI miscellaneous draw validation authority:
  `C:\WINCE600\PRIVATE\TEST\GWES\GDI\GDIAPI\draw.cpp`
- `draw.cpp::TransparentBltTransparencyTest` creates a compatible DC, selects a
  `CreateBitmap(..., NULL)` bitmap for each tested bit depth, fills the
  offscreen DC with `FillRect`, paints a black box into both source and
  destination, then calls `TransparentBlt` with `GetPixel(hdcCompat, 0, 0)` as
  the transparent key. Raw `SelectObject` now materializes owned zeroed backing
  when such a pointerless bitmap is selected into a memory DC, so subsequent
  selected-DC drawing and `TransparentImage` source reads operate on real
  storage while leaving the just-created bitmap pointerless until selection.
  Local raw coverage now sweeps the CE `CreateBitmap` bit depths and keeps the
  test backend's halfword writes coherent with byte snapshots because
  `TransparentImage` snapshots bitmap storage as bytes.
- `draw.cpp::TransparentBltErrorTest` fills the left half of the screen with a
  solid `RGB(r,g,b)` brush, then blits it into a black right half using six
  partial-channel transparent keys: `RGB(r,0,0)`, `RGB(0,g,0)`, `RGB(0,0,b)`,
  `RGB(r,g,0)`, `RGB(0,g,b)`, and `RGB(r,0,b)`. Those keys must not make the
  actual `RGB(r,g,b)` source transparent. Raw same-framebuffer `TransparentImage`
  now covers that near-miss rejection with a deterministic nonzero RGB color.
- `draw.cpp::ClipBitBlt(ETransparentImage)` uses the shared clipping matrix with
  `TransparentBlt(..., RGB(0,0,0))`, including destination rectangles that begin
  off the left/top/right/bottom clipping planes. Raw selected-DIB and
  same-framebuffer `TransparentImage` now cover all four destination clipping
  edges plus all four two-edge corner cases, fully clipped top-left/bottom-right
  no-op cases, one-pixel-visible top-left/bottom-right near-full clips, and the
  individual left/right/top/bottom fully clipped plus one-pixel-visible edge
  cases: left/top clipping advances the sampled source coordinate and row
  together, while right/bottom clipping trims the visible tail without sampling
  past the edge, fully eliminated destinations return success without painting
  or dirtying the framebuffer, one-pixel survivors map to the expected source
  edge/corner and dirty only that pixel, and visible transparent source pixels
  still preserve the destination.
