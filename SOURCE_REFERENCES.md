...
- `draw.cpp::ClipBitBlt(ETransparentImage)` uses the shared clipping matrix with
  ...
  edges plus all four two-edge corner cases and fully clipped top-left/bottom-right
  no-op cases: left/top clipping advances the sampled source coordinate and row
  ...
  or dirtying the framebuffer, and visible transparent source pixels still
