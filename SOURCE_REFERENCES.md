...
- `draw.cpp::ClipBitBlt(ETransparentImage)` uses the shared clipping matrix with
  ...
  edges plus the simultaneous top-left corner case: left/top clipping advances
  the sampled source coordinate and row together, while right/bottom clipping
  trims the visible tail without sampling past the edge, and visible
  transparent source pixels still preserve the destination.
