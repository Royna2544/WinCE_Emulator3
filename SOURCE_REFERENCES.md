...
- `draw.cpp::ClipBitBlt(ETransparentImage)` uses the shared clipping matrix with
  ...
  off the left/top/right/bottom clipping planes. Raw selected-DIB
  `TransparentImage` now covers the off-left case where clipping the destination
  must also advance the sampled source coordinate while still skipping source
