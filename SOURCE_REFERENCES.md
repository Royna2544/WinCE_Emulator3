...
- `draw.cpp::ClipBitBlt(ETransparentImage)` uses the shared clipping matrix with
  ...
  same-framebuffer `TransparentImage` now cover the off-left case where clipping
  the destination must also advance the sampled source coordinate while still
  skipping source pixels that match the transparent color.
