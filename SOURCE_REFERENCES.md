...
- `draw.cpp::ClipBitBlt(ETransparentImage)` uses the shared clipping matrix with
  ...
  no-op cases, and one-pixel-visible top-left/bottom-right near-full clips:
  left/top clipping advances the sampled source coordinate and row together,
  while right/bottom clipping trims the visible tail without sampling past the
  edge, fully eliminated destinations return success without painting or dirtying
  the framebuffer, one-pixel survivors map to the expected source corner and
  dirty only that pixel, and visible transparent source pixels still preserve
  the destination.
